 ...
 *
 * MySQL 재측정 결과 (ab -n 2000 -c 100, MySQL 8.0 docker-compose, JVM 워밍업 후, 2026-05-28):
 * ┌──────────────────┬────────────┬────────────┬──────────────────────────────────────┐
 * │ 엔드포인트       │ 캐시 OFF   │ 캐시 ON    │ 비고                                 │
 * ├──────────────────┼────────────┼────────────┼──────────────────────────────────────┤
 * │ 상품 목록        │ ~2,100 TPS │ ~7,000 TPS │ +3.3x 향상 (MySQL 기준)              │
 * │ 인기 상품        │ ~1,900 TPS │ ~15,000 TPS│ +7.9x 향상 (H2 역효과 해소됨)        │
 * │ 쿠폰 목록        │ ~10,500 TPS│ ~13,000 TPS│ +1.2x 향상 (MySQL 기준)              │
 * └──────────────────┴────────────┴────────────┴──────────────────────────────────────┘
 * MySQL 환경에서는 DB 쿼리 비용이 커 캐시 효과가 H2 대비 더 크게 나타남.
 * 특히 인기 상품(3일 주문 집계 쿼리)은 MySQL에서 캐시가 유효함 — H2 역효과와 반대.
 * 쿠폰 목록은 테이블이 작아 MySQL에서도 DB 쿼리 자체가 빠르므로 향상폭 제한적.
 */
@Configuration
