...
### 판매자 / 운영자
- 상품 재고를 정확하게 관리하여 실제 재고보다 더 많이 팔거나 재고 부족 상황을 방지해야 함
- 동시 주문 환경에서 일관성 있게 재고를 차감해야 함
- 쿠폰을 활용하여 프로모션을 진행하고 선착순 발급을 제어할 수 있어야 함
- 주문 데이터를 정확하게 외부 시스템으로 전송해야 함

### 시스템 관리자
- 주문과 재고 처리의 정합성을 보장해야 함
- 외부 시스템 연동 실패 시에도 주문 데이터의 무결성을 유지해야 함
- 높은 동시성 환경에서 안정적으로 처리해야 함
