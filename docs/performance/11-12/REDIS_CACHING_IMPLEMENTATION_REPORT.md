...
> ⚠️ **이 섹션의 모든 수치는 JMeter/nGrinder 등 부하 테스트 도구로 실측되지 않은 추정값입니다.**
> 레이턴시 개선율(4.2절 캐시 히트 시 측정값)과 이론적 계산을 조합하여 산출했습니다.

```
동시 사용자 100명, 5분 지속 테스트 기준 (추정):

┌─────────────────────────────────────────────────┐
│ 메서드별 TPS 향상 효과 (모두 추정값)           │
├─────────────────────────────────────────────────┤
│                                                  │
│ 1. getUserCoupons()                             │
│    Before: 100 TPS (추정)                       │
...
│                                                  │
│ 4. getPopularProducts()                         │
│    Before: 150 TPS (추정 — 배치 미적용)        │
│    After:  1,500 TPS (추정 — 배치 + 캐시)      │
│    향상율: 1,000% (10배) 🚀 (추정)             │
│                                                  │
├─────────────────────────────────────────────────┤
│ 전체 시스템 TPS (추정)                          │
